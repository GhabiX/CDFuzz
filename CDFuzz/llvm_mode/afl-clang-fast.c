...
static void edit_params(u32 argc, char **argv) {
    ...
    if (!strcmp(name, "afl-clang-fast++")) {
        ...
        // TODO
        cc_params[0] = alt_cxx ? alt_cxx : (u8 *)"/usr/lib/llvm-13/bin/clang++";
    } else {
        u8 *alt_cc = getenv("AFL_CC");
        cc_params[0] = alt_cc ? alt_cc : (u8 *)"/usr/lib/llvm-13/bin/clang";
    }
    ...
    cc_params[cc_par_cnt++] = "--ld-path=/usr/lib/llvm-13/bin/ld.lld";
    ...
    while (--argc) {
        ...
        if (!strcmp(cur, "-Wl,-z,defs") || !strcmp(cur, "-Wl,--no-undefined")) continue;
        ...
        if (!strcmp(cur, "-o")) binary_flag = 1;

        cc_params[cc_par_cnt++] = cur;
    }

    if (binary_name) {
        setenv("AFL_TARGET_BINARY", binary_name, 1);
        printf("[DEBUG]clang:%s\n", binary_name);
    }
...
int main(int argc, char **argv) {
    if (getenv("AFL_DEBUG") && !strcmp(getenv("AFL_DEBUG"), "1")) debug = 1;
    else debug = 0;
