...
class ModuleSanitizerCoverageLTO : public PassInfoMixin<ModuleSanitizerCoverageLTO> {
public:
...
public:
    ...
    bool runOnModule(Module &M) override {
        ModuleSanitizerCoverageLTO ModuleSancov;
...
PreservedAnalyses ModuleSanitizerCoverageLTO::run(Module &M,
    ...
    if (ModuleSancov.instrumentModule(M)) return PreservedAnalyses::none();
...
bool ModuleSanitizerCoverageLTO::instrumentModule(Module &M) {
    ...
    /* Log file stream */
    if (!getenv("AFL_TARGET_BINARY")) {
        errs() << "[DEBUG]LLVM: ERROR MISSING AFL_TARGET_BINARY ENV";
        module_str = M.getName().str();
    } else {
        module_str = getenv("AFL_TARGET_BINARY");
    }
    errs() << "[DEBUG]LLVM: " << getenv("AFL_TARGET_BINARY") << "\n";
    ...
    for (auto &F : M) {
        for (auto &BB : F) {
            ...
            Value *MapPtrIdx = IRB.CreateGEP(IRB.getInt8Ty(), MapPtr, IRB.CreateXor(PrevLocCasted, CurLoc));
...
static void registerLTOPass(const PassManagerBuilder &,
    ...
    PM.add(new ModuleSanitizerCoverageLegacyPass());
}

static RegisterStandardPasses RegisterAFLPass(
    PassManagerBuilder::EP_EarlyAsPossible, registerLTOPass);
...
static RegisterStandardPasses RegisterAFLPassLTO(
    PassManagerBuilder::EP_FullLinkTimeOptimizationEarly, registerLTOPass);
